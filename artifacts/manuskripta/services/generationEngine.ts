...
export function stopAllJobs() {
  for (const [, controller] of activeJobs.entries()) {
    controller.abort();
  }
  activeJobs.clear();
}
  ...
  try {
    ...
    for (let i = startChapter; i < totalChapters; i++) {
      if (Date.now() - startTime > BOOK_TIMEOUT_MS) {
        ...
        updateJob(job.id, {
          status: "failed",
          errorMessage: "Book generation timed out after 7 hours",
        });
        return;
      }
      ...
      updateJob(job.id, {
        currentChapter: i + 1,
        progress: Math.round((i / totalChapters) * 90),
      });
      ...
      if (!success) {
        updateJob(job.id, {
          status: "failed",
          errorMessage: `Failed to generate Chapter ${i + 1} after 3 attempts`,
        });
        log(`Chapter ${i + 1} failed after all retries`, "error");
        return;
      }
      ...
      const summary = await retry(
        () => summarizeChapter(chapterTitle, chapterContent),
        2,
        log
...
export async function runFormatJob(
  job: BookJob,
  updateJob: UpdateJobFn,
  addLog: AddLogFn
) {
  ...
  try {
    ...
    const progressAfterSection = () => {
      sectionsDone++;
      return Math.round(5 + (sectionsDone / totalSections) * 90);
    };
    ...
    updateJob(job.id, { progress: progressAfterSection() });
    ...
    for (let i = 0; i < formatData.chapters.length; i++) {
      if (Date.now() - startTime > FORMAT_TIMEOUT_MS) {
        ...
        updateJob(job.id, {
          status: "failed",
          errorMessage: "Formatting timed out after 30 minutes",
        });
        return;
      }
    ...
    const markdown = await assembleFinalFormattedBook(
      formatData.bookTitle,
      formatData.copyright,
      formatData.dedication,
      formattedIntro,
      formattedChapters,
      formattedConclusion,
      formattedBackMatter,
...
async function retry<T>(
  fn: () => Promise<T>,
  maxAttempts: number,
  log: (msg: string, type: JobLog["type"]) => void
): Promise<T> {
